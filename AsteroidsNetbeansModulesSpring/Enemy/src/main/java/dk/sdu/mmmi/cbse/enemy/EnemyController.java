...
public class EnemyController implements IEntityProcessingService
{
	...
	public void process(GameData gameData, World world)
	{
		for(Entity enemy : world.getEntities(Enemy.class))
		{
			...
			// Dice roll
			int dizeSize = 20;
			int rolledDice = (int) Math.round(Math.random() * (dizeSize - 1) + 1);
			if(rolledDice == dizeSize) // Critical!
			{
				dizeSize = 3;
				rolledDice = (int) Math.round(Math.random() * (dizeSize - 1) + 1);
				switch(rolledDice)
				{
					case 1:
			...
			if (bl == null)
			{
				...
				bl = (BulletLookup) ac.getBean("bulletLookup");
			}
			
			for	(BulletSPI bullet : bl.getBulletSPI())
			{
				Entity actualBullet = bullet.createBullet(enemy, gameData);
				world.addEntity(actualBullet);
			}
